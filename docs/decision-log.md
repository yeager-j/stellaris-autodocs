...
That stand-in is the one thing the harness must not let a reader misread, because until Phase 6 **the only values a corpus contributes to a published revision are its two `RevisionInputs` fingerprints** — nothing parses the bytes, and an identifier appearing both in a fixture file and in an entry summary is the author typing it twice. A doc comment alone would survive Phase 6 and start lying, so the claim is asserted: two corpora with identical documentation over different bytes must produce differing fingerprints and identical served entries, and that test is scheduled to go red in Phase 6. It deliberately does not compare the two published revision identifiers, which would pass whatever the corpus contributed — two threads mint two Discovery Location identifiers, so their installations differ before a fingerprint is consulted. That the input fingerprints reach revision identity is `revisions::manifest`'s `every_identity_component_changes_the_identifier`, which makes the claim one varying component at a time — the only way to make it unconfounded.
