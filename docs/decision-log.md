...
**D-137 — `localization` owns its ingestion input; `analysis` builds one in a single adapter**

Recorded 2026-07-30 with STE-37 (Phase 5A), which landed after STE-38 and STE-39. The resolver's localization handoff — `LocalizationFileStream`, `LocalizationFile`, `ShadowedLocalizationFile` — is `pub(in crate::analysis)`, and `localization` is a sibling top-level module, so who owns the shape the bytes cross in had to be decided rather than inherited.

**The decision.** `localization` declares the question. `LocalizationInput` names only crate-public primitives (`SourceKind`, `LogicalPath`, `SourceBytes`) plus localization-owned types, and `analysis::localization_stage` builds one from a `Resolution`. The resolver is untouched: it still stops before `.yml` interpretation, and it still produces exactly one output vocabulary.

**Rejected.** Widening the handoff types to `pub(crate)` publishes the resolver's five-kind provenance vocabulary as a cross-module contract to buy one struct, and closes a `localization -> analysis` edge against the one sanctioned direction — the shape D-122 withdrew a claimed edge rather than accept. Re-homing the handoff type into `localization` either keeps `FactProvenance`, which is not visible outside `analysis`, or drops it — in which case it is `LocalizationInput` under another name but constructed inside the resolver, giving the resolver two output vocabularies and a `crate::localization` import it has no business holding. Letting `localization` call the resolver inverts the sanctioned direction outright.

**Localization owns its key-level provenance vocabulary, and that is still one authority.** `FactSite` cannot express it: it has no line, and a line is the only thing that orders two statements of one key inside one file; its `StreamPosition::ordinal` already means "definition ordinal within its file" and is hashed under that meaning by the conformance digest; a removed file has no stream position at all, so occurrences in shadowed files could not be ordered against surviving ones; and it has no notion of a language section, while `localisation/languages.yml` holds ten in one file. Three of the five `FactKind`s — `Inherited`, `Defaulted`, `Duplicate` — have no meaning for a localization key. So the authority splits rather than duplicating: **file selection is the resolver's fact** (which file lost and why), carried across once as `FileLoss` by one exhaustive `match` and never re-derived; **key attribution is localization's fact**, and nothing else in the crate can compute it because nothing else reads `.yml`.

**Enforced, not described.** Visibility already blocks one direction, but it is contingent on nobody widening it — and widening it is exactly the moment the decision is being reversed. Two text scans notice the *name* instead, each with a seeded negative control run through the same scanning function: `localization::tests::localization_names_no_analysis_type` forbids shipped `localization` code from naming an `analysis` type or the Clausewitz parser, and `localization_stage::tests::only_the_stage_builds_localization_tables` forbids shipped `analysis` code outside the stage from calling `localization::ingest`. Both are scoped to shipped code, the second deliberately: the r15 key-level expectation must build a stream production cannot produce, because production supplies one Target Mod rank and the record measured two.

**D-138 — Localization's unmeasured lexical rules are corpus-measured with a committed census, and its unmeasurable ones are typed conditions**

Recorded 2026-07-30 with STE-37. No oracle record measures `.yml` lexis, and Phase 5A had to settle four questions to read a single key.

**Measured, and asserted by [the lexical census](./spikes/localization-lexical-census.md).** The `:<version>` suffix is optional and discarded — 92.8% of vanilla key lines omit it, and nothing downstream reads one. A value runs from the first `"` to the **last** `"` on the line: 1,477 vanilla lines carry unescaped inner quotes, so ending at the next quote truncates player-visible prose, and 12,702 carry a trailing `#` comment, so scanning to end of line swallows one. A byte-order mark is stripped if present and never required. Locale identity comes from the `l_<language>:` section header and never from the path — Workshop mod `3039370479` ships `localisation/braz_por/PreSelect_l_braz_por.yml` under an `l_english:` header — and a header switches the current language mid-file, because `languages.yml` holds ten sections.

**`localization_interpretation` moves to 2, and that closes D-136's deferral too.** STE-38 left it at 1 because nothing had been published under any tokenization semantics and the number had no ingestion behind it. Ingestion is what gives it one: locale identity, the line grammar, the value rule and the merge order all decide what a build reads out of a mod, so the same inputs now produce different documentation than they did at 1. The constant is homed in `localization` beside the rules it versions and quoted by the analysis version vector, which is what makes D-136's "a later change to these marker rules must bump it" enforceable rather than remembered — the marker rules and the lexical rules are versioned by one number, in one place, from here on.

**The value rule's residual is bounded rather than argued away.** A trailing comment that itself contained a quote would be swallowed silently. The census asserts zero such values and zero lines with non-comment text after the last quote, across 1.5 million lines. If either fails, the rule becomes "the last quote before an unquoted `#`" and `LOCALIZATION_INTERPRETATION_VERSION` bumps with it.

**Unmeasurable, so typed.** What the game does with a malformed line is unknown — no record covers it. Ingestion drops the line and records `UnterminatedValue`, `UnquotedValue`, or `Unparsable` with its file and line, because inventing a value fabricates a player-visible name and refusing the build over three lines in one mod would refuse documentation for the other sixteen thousand keys. The first two also carry the **key** the line named, on the standard `r12` set for the resolver's typed facts: a condition that did not name what failed could not be surfaced as an Analysis Issue without reparsing the source, and "the definition of `x` is malformed" is the whole content of the issue. `Unparsable` carries none, and the asymmetry is the point — it is the one shape where the parse could not identify a key, so offering one would be offering a guess. A header the game itself could not read as a language — `l_English:`, `l_日本語:` — skips its section and counts what it skipped rather than being guessed at. Invalid UTF-8 costs the whole file and is never lossily converted: a name rendered with `U+FFFD` is fabricated, and a key that renders raw is at least honestly raw.

**The one language vocabulary, collapsed as D-135's merge rule directs.** STE-37 landed second, so ingestion parses `.yml` section headers through STE-39's `LanguageTag` and the closed ten-variant enum this ticket first wrote is gone. The collapse is an improvement and not merely a merge: a syntactic parse means a translation mod's own language gets a table, where the closed set would have classified every community translation's keys as skipped — the failure mode `LanguageTag`'s own doc comment names. Three consequences follow. The unknown-language condition narrows to *unreadable header* and is vacuous over both corpora, so it rests on unit tests rather than on a corpus count. The set of languages a build offers is now derived from the tables rather than declared, which is what the read side wanted anyway. And the census's language claim changes from a constraint into a reading of the pinned build: exactly the ten `languages.yml` declares, asserted because a header-parsing regression would shrink that set long before any key count looked wrong, and failing loudly — correctly — if the game ever ships an eleventh.

**A raw-key casualty is a value, not an absence.** `r14` measured that a same-named file blanks its vanilla counterpart's other keys, and that those keys render in game as their own identifiers. So a key stated only in removed files is enumerated with the statements it lost and the file that took them, and it is kept *out* of the effective entries: a key with no winner has no value, and putting a stand-in there is precisely the "raw key presented as a name" failure the design forbids. `entries.get(k) == None` with a casualty means "the player sees the identifier, and here is why"; `None` with no casualty means nobody ever stated it. Keeping those apart is what the fallback chain (STE-40) needs to distinguish a shadowing accident from an undefined key, and what an Analysis Issue needs to tell a deliberate rename from hundreds of casualties.
