...
- `revisions -> discovery` for the Mod Installation and Discovery Location identifiers a revision manifest and a publication reference are keyed by. Owner: `discovery`, which mints both identifiers and owns their derivation and rendered form. Cycle check: `discovery` depends only on `canonical` and `error`, never on `revisions`; nothing discovery does needs a bundle, and a bundle exists only after discovery has finished. The identifiers are held as the types `discovery` minted rather than restated as private hex newtypes, so a manifest's spelling of an installation identifier cannot drift from `discovery`'s.
- `revisions -> source` for the `SourceFingerprint` and `ContentHash` values that are the exact input fingerprints a manifest records and the Revision identifier is derived over. Owner: `source`, which owns fingerprint construction and the enumeration policy whose version the fingerprint quotes. Cycle check: `source` depends only on `canonical` and `error`, and is already the target of `analysis -> source` for the same class of reason; it has no path back to `revisions`.
- `revisions -> analysis` for the `AnalysisVersionVector` a manifest records and the Revision identifier nests as one digest. Owner: `analysis`, which owns the vector's components and the schedule on which they change. Cycle check: acyclic because `analysis` acquires no dependency on `revisions` — it never names a Revision Candidate, a bundle, or a manifest. The Revision Candidate is assembled by the application layer, which sits above both and already holds analysis's output.
- `companion -> revisions` for authorized revision opening.

Each of the three `revisions` identity edges is a stored identity value the manifest holds and the identifier is derived over, not a behavioral dependency: `revisions` calls no traversal, no source hashing, and no analysis.
