...
- **Spike code migration:** `tools/` harnesses are mined for validated logic (parser adaptation, DDS classification, bundle round-trips) but production modules are written fresh behind their designed interfaces. A spike is deleted once the work it de-risked lands; a capability that outlives its spike is ported into owned code first, never kept alive inside the spike. Concretely: the oracle runner and DDS cross-check stay in `tools/` as verification tooling; the parser spike's independent tape reading is ported into the Phase 4 local-corpus conformance harness as a production-owned dev-only projection (re-run on a Stellaris build change, a Jomini upgrade, or a dialect-lexer edit — the same standard the DDS recipe holds for decoder upgrades), and `tools/parser-spike` was deleted when that harness landed (Phase 4, task 3), taking `tools/bundle-spike` with it since Phase 3 had already landed what that one de-risked and it held a path dependency on the parser spike. Captured records under `docs/spikes/` remain as evidence in every case, each evaluation carrying a provenance note naming the harness that produced it.
